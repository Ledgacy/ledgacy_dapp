...
contract Ledgacy {
    ...
    function getEncryptedKeypartCount() public view returns (uint) {
        return encrypted_keyparts.length;
    }

    function getEncryptedKeypart(uint index) public view returns (bytes){
        return encrypted_keyparts[index];
    }
